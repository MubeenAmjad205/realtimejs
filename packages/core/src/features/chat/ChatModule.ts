...
export function createChatModule(eventRouter: EventRouter, registry: AdapterRegistry, permissions?: ChatPermissions) {
  ...
  eventRouter.on('chat:message', (payload: Message) => listeners.forEach(l => l(payload)));
  eventRouter.on('chat:message_edited', (payload: Partial<Message>) => editListeners.forEach(l => l(payload)));
  eventRouter.on('chat:message_deleted', (payload: { messageId: string }) => deleteListeners.forEach(l => l(payload.messageId)));
  eventRouter.on('chat:reaction_changed', (payload: any) => reactionListeners.forEach(l => l(payload)));
  eventRouter.on('chat:message_status', (payload: any) => statusListeners.forEach(l => l(payload)));
